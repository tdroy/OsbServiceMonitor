...
# Main
domainConfig()
FMW_Version = get('DomainVersion')
print "FMW version : " + FMW_Version
...
tableHeader = ["Project","ServiceName", "Stats-Name","Min-Resp","Max-Resp","Avg-Resp","Sun-Resp","Status","Action"]
tableData = []
...
for ref in allRefs:
	...
	if typeID == "ProxyService":
		...
		#props = cmo.getStatistics([ref],[ResourceType.SERVICE.valueOf("SERVICE")],'')        
		...
		for rs in props[ref].getAllResourceStatistics():            
			for e in rs.getStatistics():
				...
				if e.getType() == StatisticType.INTERVAL and e.getName() == "response-time":
					...
					strTable.append(str(e.getAverage()))
					strTable.append(str(e.getSum()))
					strTable.append(serviceStatus)
					strTable.append(action)
					tableData.append(strTable)	
		print ""

TableIt.printTable(tableData, useFieldNames=True)
